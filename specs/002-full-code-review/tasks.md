...
- [X] T078a [P2-SEC] Follow-up to T078 (filed 2026-04-19): add `validate_collab_name(name)` to `bot/collaborative.py` (reject empty / control chars / path separators / `.` / `..`; enforce `^[a-z0-9][a-z0-9-]*$`, ≤64 chars). Call from `_handle_collab_announce` before the agent-file `write_text`, and from `CollabStore.create_pending` for defense-in-depth. Rejection in the announce path surfaces via the existing `collab_announce_error` STRING. Tests: traversal name rejected, control chars rejected, valid name accepted, existing flows unchanged. Closes **P2-81**. — **closed 2026-04-19**: shared validator in `bot/collaborative.py` matches the alphabet Flow B already produces via `_sanitize_task_name` so it introduces zero behaviour change for legitimate names; wired into both `_handle_collab_announce` (rejection surfaces via `collab_announce_error`) and `CollabStore.create_pending` (defense-in-depth); +39 parametrised regression tests (19-case validator + 6-case create_pending + 13-case handler + existing empty-name test retargeted).
- [X] T079 [P] [P2-SEC] Security-audit `bot/_bootstrap.py` + `bot/process.py`: import-time side effects, subprocess lifecycle guarantees, zombie reaping; tests under `tests/test_bootstrap.py` and `tests/test_process.py` — **closed 2026-04-19**: closed **P2-86** (`_bootstrap.py::ensure_dependencies` now spawns pip with `env=_scrubbed_child_env()` stripping the same 7 tokens/keys as `bot/updater.py::_CHILD_ENV_SCRUB` — mirrors P2-71 on the updater, applied on the startup path; stdlib-only helper preserves the file's no-third-party-imports constraint; +2 regression tests, including a scrub-list-parity assertion to catch divergence from `updater.py`). Other lenses verified clean: both modules are side-effect free at import; every subprocess uses argv lists (no `shell=True`); timeouts set on every subprocess (600 s pip, 5 s ps/tasklist); `subprocess.run` + `asyncio.wait_for(proc.communicate())` reap reliably — no zombie PIDs; `process.py` formats `pid` via `%d`/`str(pid)` from int-only internal sources. Low-priority observation noted: `_bootstrap.py` uses `capture_output=True` on pip — real-world output is small, not worth the streaming refactor.
...
- [X] T119 Bump `VERSION` (patch bump unless a migration was introduced) and add a migration entry under `bot/migrations/` via `scripts/new_migration.py` only if a schema change was actually made — **closed 2026-04-19**: patch bump `0.22.1 → 0.22.2`; no schema change, so no new migration file required (last migration remains `v0_22_1`).
- [X] T120 Create `releases/vX.Y.Z.md` summarizing Pass 2: findings fixed count, deferred-with-rationale count, test-count delta, LOC delta — **closed 2026-04-19**: `releases/0.22.2.md` published with category-by-severity breakdown (22 fixed + 1 noted/test-guarded), in-cycle fix list (P2-80/81/82/83/84/86 + T068/T069/T073/T079 audit closures), deferred-with-rationale table for Pass 1 F* + P2-STB/UX/NI remainders, and test/LOC delta (1451 → 1532 passing).
- [X] T121 Append Pass 2 section to `CHANGELOG.md` — **closed 2026-04-19**: `CHANGELOG.md` 0.22.2 entry prepended, mirroring the release notes structure (Security/Fixed + Audit closures + Added + Deferred + Tests).
- [X] T122 Run `quickstart.md` §8 close-out sequence end-to-end; do NOT push without explicit user request — **closed 2026-04-19**: §8 sequence complete: (1) `pytest tests/ -q` → 1532 passed, baseline exceeded by +81; (2) every Pass 2 row fixed/deferred-with-rationale; (3) Pass 1 F12/F13/F14/F17/F20/"All adapters" dispositions recorded in findings.md close-out section; (4) VERSION 0.22.2; (5) `releases/0.22.2.md` published; (6) final commit `c775f61` lands on branch `002-full-code-review`, **not pushed** per policy. Branch sits 14 commits ahead of `main`.
