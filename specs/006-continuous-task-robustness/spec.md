...
- **Topic deletion between sessions**: if a user manually deletes a dedicated continuous-task topic, the next scheduler tick detects detachment and either recreates the topic or marks the task "detached — action required" in the journal and sends one message to HQ as the last-resort surface.
...
- **FR-001**: The system MUST record every continuous-task lifecycle event (creation, dispatch, step-start, step-complete, state transition, stop, resume, complete, delete, error, orphan incident, rate-limit enter/exit, lock recovery, drain-on-close) to an append-only event journal with, at minimum: ISO-8601 timestamp, task name, event type, outcome, and a small structured payload.
- **FR-002**: The system MUST NOT push any message into HQ as a direct side-effect of scheduler dispatch activity, lock maintenance, orphan detection, or routine state transitions.
- **FR-003**: The orchestrator agent in HQ MUST be able to query the event journal on demand — filtered at minimum by time window and optionally by task name or event type — and receive a chronologically ordered summary suitable for rendering to the user.
...
- **FR-021**: When the parent workspace of a continuous task is closed while a step is running, the system MUST either (a) allow the step to finish within a bounded drain window (default: 30 minutes) and deliver its output with a "workspace closed mid-step" marker to the task's dedicated topic, or (b) if the topic is being deleted as part of closure, archive the step output and post a single archive-reference message to a configured fallback target.
...
- **SC-001**: With three concurrent continuous tasks running for 2 hours (each producing ≥1 step), HQ receives zero automatic scheduler-related messages.
...
- One-shot and periodic task paths are out of scope unless a shared helper (lock heartbeat, event journal) naturally covers them as a side benefit.
