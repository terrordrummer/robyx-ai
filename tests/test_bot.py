...
class TestReminderEngine:
    @pytest.mark.asyncio
    async def test_run_reminder_cycle_uses_check_reminders(self, mock_platform, tmp_path):
        reminders_file = tmp_path / "data" / "reminders.json"
        reminders_file.parent.mkdir(parents=True, exist_ok=True)

        with patch.object(bot, "reminders_file_path", return_value=reminders_file), \
             patch("reminders.check_reminders", new_callable=AsyncMock) as mock_check:
            await bot.run_reminder_cycle(mock_platform, default_chat_id="C01CONTROL")

        mock_check.assert_awaited_once_with(
            reminders_file=reminders_file,
            platform=mock_platform,
            default_chat_id="C01CONTROL",
        )

    @pytest.mark.asyncio
    async def test_reminder_job_forwards_job_data(self, mock_platform, tmp_path):
        reminders_file = tmp_path / "data" / "reminders.json"
        context = _make_job_context(mock_platform)
        context.job.data["default_chat_id"] = -100999
        context.job.data["reminders_file"] = reminders_file

        with patch.object(bot, "run_reminder_cycle", new_callable=AsyncMock) as mock_cycle:
            await bot.reminder_job(context)

        mock_cycle.assert_awaited_once_with(
            mock_platform,
    ...
    @pytest.mark.asyncio
    async def test_slack_loop_runs_reminders_with_control_room_channel(self, mock_platform):
        mock_platform.control_room_channel = "C01CONTROL"
        sleep = AsyncMock(side_effect=[None, RuntimeError("stop")])

        with patch.object(bot, "run_reminder_cycle", new_callable=AsyncMock) as mock_cycle, \
             patch.object(bot.asyncio, "sleep", sleep):
            with pytest.raises(RuntimeError, match="stop"):
                await bot._slack_reminder_loop(mock_platform)
    ...
    @pytest.mark.asyncio
    async def test_discord_loop_runs_reminders_with_control_room_id(self, mock_platform):
        mock_platform.control_room_id = 444555666
        client = MagicMock()
        ...
        with patch.object(bot, "run_reminder_cycle", new_callable=AsyncMock) as mock_cycle, \
             patch.object(bot.asyncio, "sleep", sleep):
            with pytest.raises(RuntimeError, match="stop"):
                await bot._discord_reminder_loop(mock_platform, client)

        client.wait_until_ready.assert_awaited_once()
        mock_cycle.assert_awaited_once_with(
            mock_platform,
    ...
    @pytest.mark.asyncio
    async def test_background_timed_scheduler_loop_passes_platform(self, mock_platform):
        sleep = AsyncMock(side_effect=[None, RuntimeError("stop")])

        with patch.object(bot, "run_timed_cycle", new_callable=AsyncMock) as mock_cycle, \
             patch.object(bot.asyncio, "sleep", sleep):
            with pytest.raises(RuntimeError, match="stop"):
                await bot._background_timed_scheduler_loop(mock_platform, MagicMock())

        mock_cycle.assert_awaited_once_with(ANY, platform=mock_platform)
