...
- `bot/events.py` is the append-only spec-006 event journal at `data/events.jsonl` with hourly + 10 MB size-based rotation to `data/events/events-YYYYMMDD-HH.jsonl` and 7-day retention. `bot/handlers.py::_handle_get_events` expands `[GET_EVENTS since="…" task="…" type="…" limit="…"]` macros into compact markdown tables. Contract at `specs/006-continuous-task-robustness/contracts/events-macro.md`.
- `bot/continuous_state_machine.py` formalises the spec-006 continuous-task lifecycle states (`pending / running / awaiting_input / rate_limited / stopped / completed / error / deleted`) with transition validation and legacy-alias normalisation.
